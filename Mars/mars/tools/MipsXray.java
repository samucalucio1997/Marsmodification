...
//      	   JPanel jp = new JPanel(new FlowLayout(FlowLayout.LEFT));
   	...
   	protected AccessNotice processMIPSUpdate(Observable resource, AccessNotice notice) {

   		if (!notice.accessIsFromMIPS()) return notice;
   		if (notice.getAccessType() != AccessNotice.READ) return notice;
   		MemoryAccessNotice man = (MemoryAccessNotice) notice;
   		int currentAdress = man.getAddress();

   		if (currentAdress == lastAddress) return notice;
